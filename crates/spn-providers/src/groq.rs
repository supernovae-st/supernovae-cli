//! Groq cloud backend.
//!
//! Groq provides ultra-fast inference using LPUs (Language Processing Units).
...
impl GroqBackend {
    ...
    /// Create a new Groq backend with a custom endpoint.
    pub fn with_endpoint(
        ...
        if api_key.is_empty() {
            return Err(BackendsError::MissingApiKey(BackendKind::Groq));
        }
    ...
    pub fn from_env() -> Result<Self, BackendsError> {
        let api_key = std::env::var("GROQ_API_KEY")
            .map_err(|_| BackendsError::MissingApiKey(BackendKind::Groq))?;
        Self::new(api_key)
    }
...
impl CloudBackend for GroqBackend {
...
impl DynCloudBackend for GroqBackend {
    fn kind(&self) -> BackendKind {
        BackendKind::Groq
    }
...
mod tests {
    ...
    #[test]
    fn test_groq_backend_creation() {
        let backend = GroqBackend::new("gsk_test_key_123").unwrap();
        assert_eq!(CloudBackend::kind(&backend), BackendKind::Groq);
    ...
    #[test]
    fn test_groq_backend_empty_key() {
        let result = GroqBackend::new("");
        assert!(result.is_err());
        match result {
            Err(BackendsError::MissingApiKey(kind)) => {
                assert_eq!(kind, BackendKind::Groq);
            }
    ...
    fn test_convert_messages() {
        ...
        let groq_messages = GroqBackend::convert_messages(&messages);
